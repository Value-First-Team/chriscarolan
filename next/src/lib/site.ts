/**
 * Site constants for chriscarolan.com.
 *
 * Ported verbatim from the Astro version (src/lib/site.ts). The links here drive
 * this node's own in-body CTAs (Book a Call, Book to Speak, Speaking Kit, etc.)
 * — distinct from the shared constellation shell's nav/footer config
 * (src/components/navigation/config.ts).
 */

export const SITE = {
  name: 'Chris Carolan',
  url: 'https://chriscarolan.com',
  description:
    'Chris Carolan — business transformation advisor for the AI era. Founder of Value-First Team. Helping leaders rethink how their companies operate.',
  tagline: 'Rethinking how businesses operate in the AI era',
  email: 'chris.carolan@valuefirstteam.com',
  social: {
    linkedin: 'https://www.linkedin.com/in/chris-carolan/',
    x: 'https://x.com/chriscarolan',
    instagram: 'https://www.instagram.com/beardabides/',
    youtube: 'https://www.youtube.com/@Value-First',
  },
  links: {
    bookCall: 'https://meetings.hubspot.com/chris-carolan1/value-first-team-free-consult',
    valueFirstTeam: 'https://www.valuefirstteam.com',
    shows: 'https://www.valuefirstteam.com/shows',
    aiDailyShow: 'https://ainativeshift.com',
    // Public speaking booking — HubSpot Meetings (per Chris, 2026-05-14).
    bookToSpeak: 'https://meetings.hubspot.com/chris-carolan1/public-speaking',
    // Speaking & Media Kit — hosted PDF (Brand Alchemy final deliverable).
    speakingKit: '/speaking-media-kit.pdf',
    // "Surviving the SaaSpocalypse" book row → Learn More CTA. Temporary target
    // per Chris 2026-06-09 ("for now"); revisit when the book page is final.
    bookLearnMore: 'https://www.valuefirstteam.com/book/saaspocalypse',
  },
} as const;
